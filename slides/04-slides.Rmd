...
# The data needs to be reshaped a bit first
gapminder |>
  filter(year == 2007) |>
  waffle_iron(aes_d(group = continent), rows = 9) |>
  ggplot(
    # x and y are switched here so the boxes go rowwise
    aes(x = y, y = -x, fill = group)
  ) +
  geom_waffle(linewidth = 0.25) +
  labs(fill = NULL) +
